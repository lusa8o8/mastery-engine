...
test('tutor question IDs and extraction retries are durable', async () => {
  ...
  const uploadPage = await read('src/pages/UploadPage.jsx')
  const extractionClient = await read('src/utils/extractQuestions.js')
  ...
  assert.match(extractionClient, /supabase\.auth\.refreshSession\(\)/)
  assert.match(extractionClient, /!session\?\.access_token/)
  assert.match(uploadPage, /\['pending', 'failed'\]\.includes\(existing\.extraction_status\)/)
  assert.match(uploadPage, /const paper = existing \|\| await uploadPaper/)
